...
def update_user(
    ...
    updated = user_in.dict(exclude_unset=True)
    for key, value in updated.items():
        setattr(user, key, value)

    log_activity(db, current_admin.id, "UPDATE", "User", user_id, {"changes": updated})
    db.commit()
    db.refresh(user)
    return user
...
def delete_user(
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    log_activity(db, current_admin.id, "DELETE", "User", user_id, {"email": user.email})
    db.delete(user)
    db.commit()
